...
public extension String {
    func toPrivateKeyFromWif() -> String {
        let decoded = try! self.decodeBase58()
        let bytes  = [UInt8](decoded)
      //  if bytes.count != 37 || bytes[0] != 0x80 /*|| bytes[33] != 0x01*/  { //may need to re add this, not sure if its NEO specific check
        //    return ""
        //}
        //Make sure we have a valid checksum
        let checkSum = bytes[bytes.count - 4...bytes.count - 1]
        let shortened = bytes[0...bytes.count - 5]
